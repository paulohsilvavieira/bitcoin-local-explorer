...
router.get("/", asyncHandler(async (req, res, next) => {
	try {
		...
		if (getblockchaininfo.blocks) {
			...
			// clamp at 0: chains younger than recentBlocksCount (e.g. a fresh regtest) would
			// otherwise produce negative heights here.
			for (let i = 0; i < (config.site.homepage.recentBlocksCount + 1) && (getblockchaininfo.blocks - i) >= 0; i++) {
...
router.get("/next-block", asyncHandler(async (req, res, next) => {
	...
	// getblocktemplate/caching podem ocasionalmente devolver um resultado sem
	// "transactions" (ex: mempool vazio ou uma corrida no cache) — não deixa
	// a página quebrar por causa disso.
	if (!blockTemplate.transactions) {
