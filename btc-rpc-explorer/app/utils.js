...
function nextHalvingEstimates(eraStartBlockHeader, currentBlockHeader, difficultyAdjustmentDataArg=null) {
	...
	if (nextHalvingIndex > terminalHalvingCount) {
		// já passamos do halving terminal (subsídio zerado) — acontece rápido em
		// regtest (intervalo de 150 blocos) com uma chain rodando por muito tempo.
		// Mantém o mesmo formato de retorno do caso normal (com valores "não tem
		// mais halving") em vez de um objeto parcial, pra não quebrar quem espera
		// esses campos (ex: views/includes/index-network-summary.pug).
		return {
			blockCount: blockCount,
			...
			daysUntilNextHalving: 0,
			nextHalvingDate: new Date(),
